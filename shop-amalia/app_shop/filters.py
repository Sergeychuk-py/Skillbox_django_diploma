...
from app_product.models import PropertyName, Product
...
class ProductFilter(PropertyFilterSet):
    ...
    is_limited = PropertyBooleanFilter(
        field_name="limited_edition", widget=ShopCheckboxInput
    )
    product_property = LookupChoiceFilter(
        field_name="product_property__property__name",
        lookup_choices=[
            (property_name, property_name)
            for property_name in PropertyName.objects.all()
        ]
        if db_table_exists("product_properties")
        else [],
    )
    product_values = LookupChoiceFilter(
        ...
        lookup_choices=[
            ...
            for property_value in PropertyValue.objects.all()
        ]
        if db_table_exists("property_value")
        else [],
    )
