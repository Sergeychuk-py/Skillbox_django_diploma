...
class ShopIndexView(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        favourite_categories: Category = Category.objects.filter(favourite=True)

        popular_products: Product = Product.objects.filter(active=True).order_by(
            "-sold_amount"
        )[: settings.popular_product_amount]
        limited_products: Product = Product.objects.filter(
            active=True, limited_edition=True
        ).order_by("price")
