...
class User(AbstractBaseUser, PermissionsMixin):
    image_validator = FileExtensionValidator(
        ...
        message=_(
            "Расширение не поддерживается. Разрешенные расширения .jpg .gif .png .jpeg"
        ),
    )
    ...
    def validate_image_size(fieldfile_obj):
        ...
        if filesize > megabyte_limit * 1024 * 1024:
            raise ValidationError(
                _("Вы не можете загрузить аватар размером > {} MB").format(
                    settings.avatar_image_max_size_mb
                )
            )
    ...
    date_joined = models.DateTimeField(
        auto_now_add=True, verbose_name=_("Дата регистрации")
    )
