...
from .models import DeliveryMethod, Order, OrderItems, PaymentMethod, PaymentStatuses
...
class OrderView(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context[
            "section_column"
        ] = " Section_column Section_columnRight Section_columnWide Order"
        return context
...
class OrderPaymentProcess(generic.TemplateView):
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        order: Order = Order.objects.filter(pk=order_id)
        if order.exists():
            order = order.first()
            context["order"] = order
            if not card_number.endswith("0"):
                ...
                new_status: PaymentStatuses = PaymentStatuses.objects.get(
                    pk=payment_error
                )
                order.payment_status = new_status
                order.save()
    ...
    if request.method == "POST" and is_ajax(request):
        ...
        for item in cart.__iter__():
            ...
            if product.remains >= amount:
                ...
                response["msg"] = _(
                    'ОШИБКА: Товара "<b>{}</b>" не осталось на складе. Проверьте корзину!'
        ...
        if len(prepared_items) > 0:
            ...
            payment_method: PaymentMethod = PaymentMethod.objects.filter(pk=payment)
            ...
            if go_next:
                ...
                for item in prepared_items:
                    ...
                    if not order_item.id:
                        ...
                        Order.objects.filter(pk=order.id).delete()
...
def cancel_order(request: HttpRequest):
    ...
    if request.method == "POST" and is_ajax(request):
        ...
        print(f"{order_id=}")
        order: Order = Order.objects.filter(pk=order_id, user=request.user)
        if order.exists():
