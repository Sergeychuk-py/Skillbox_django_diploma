    ...
    if request.method == "POST" and is_ajax(request):
        ...
        if product.exists():
            ...
            if product.remains > 0:
                ...
                cart = Cart(request)
                cart.add(
                    product=product, quantity=int(product_amount), update_quantity=False
                )
                ...
                if product_remains > product.remains:
                    ...
                    response["msg"] = _(
                        'Количество товара <b>"{}"</b> ограничено! Доступно: {} шт.'
                    ).format(product.title, product.remains)
                else:
                    response["msg"] = _(
                        'Товар <b>"{}"</b> успешно добавлен в корзину'
...
def update_cart_view(request: HttpRequest):
    """
    AJAX-REQUEST - Updating the cart in the header
    :param request:
    :return:
    """
    response = dict()
...
def update_product_amount(request: HttpRequest):
    """
    AJAX-REQUEST - Product Quantity Update
    :param request:
    :return:
    """
    response = dict()
        ...
        if product.exists():
            ...
            if product.remains > 0:
                ...
                if product_remains > product.remains:
                    ...
                    response["msg"] = _(
                        'Количество товара <b>"{}"</b> успешно обновлено ({})'
                    ).format(product.title, product_remains)
...
def remove_product(request: HttpRequest):
    ...
    if request.method == "POST" and is_ajax(request):
        ...
        if product.exists():
            ...
            response["result"] = True
            response["msg"] = _('Товар <b>"{}"</b> успешно удалён из корзины!').format(
                product.title
            )
