...
class Cart(object):
    ...
    def save_in_db(self, cart, user):
        for key, value in cart.items():
            if CartDatabase.objects.filter(user=user, good=key).exists():
                good = CartDatabase.objects.select_for_update().get(user=user, good=key)
    ...
    def get_total_price_product(self, product_id):
        ...
        if self.use_db:
            total = (
                self.qs.filter(good_id=product_id)
                .only("quantity", "price")
    ...
    def get_quantity(self, product_id):
        ...
        if self.use_db:
            ...
            quantity = qs.only("quantity").aggregate(quantity=Sum(F("quantity")))[
                "quantity"
            ]
            if not quantity:
                quantity = 0
